...
const ethereumData = async () => {
  const response = await fetch('https://min-api.cryptocompare.com/data/v2/histominute?fsym=ETH&tsym=USD&limit=119&api_key=da16ad98c7abaa53a707853afacc055dd94add255154d87f20a3d3c238bc130f');
  const json = await response.json();
  const data = json.Data.Data
  const times = data.map(obj => obj.time)
  const prices = data.map(obj => obj.high)
...
const linkData = async () => {
  const response = await fetch('https://min-api.cryptocompare.com/data/v2/histominute?fsym=LINK&tsym=USD&limit=119&api_key=da16ad98c7abaa53a707853afacc055dd94add255154d87f20a3d3c238bc130f');
  const json = await response.json();
  const data = json.Data.Data
  const times = data.map(obj => obj.time)
  const prices = data.map(obj => obj.high)
...
const dotData = async () => {
  const response = await fetch('https://min-api.cryptocompare.com/data/v2/histominute?fsym=DOT&tsym=USD&limit=119&api_key=da16ad98c7abaa53a707853afacc055dd94add255154d87f20a3d3c238bc130f');
  const json = await response.json();
  const data = json.Data.Data
  const times = data.map(obj => obj.time)
  const prices = data.map(obj => obj.high)
...
async function printlinkChart() {
  ...
  let ethereumChart = document.getElementById('linkChart').getContext('2d');
  ...
  creatlinkChart = new Chart(linkChart, {
    type: 'line',
    data: {
      labels: times,
      datasets: [{
...
async function printdotChart() {
  ...
  let ethereumChart = document.getElementById('dotChart').getContext('2d');
...
async function updatelinkPrice() {
  ...
  document.getElementById("linkPrice").innerHTML = "$" + currentPrice;
}
...
async function updatedotPrice() {
  ...
  document.getElementById("dotPrice").innerHTML = "$" + currentPrice;
}
